...
\_Register handlers on MobileWebController.Instance to receive messages in your application and use MobileWebController.Instance.SendMessage to distribute data to the web clients.
More details can be found in the [Setup](#Setup) and [Desing your frontend](#Design your frontend) sections.
A demo project which is showing the integration of this plugin is available [here](https://bitbucket.org/kaufi07/mobile-web-control/src/master/). (Have a look at Assets/Scripts/Network/ for the Unity application code and Assets/WebResources for the frontend implementation)

## Setup

Firstly, if you want to use the default server implementations, you need to create a script extending the abstract MobileWebControlInitializer, which will set up the network communication with your network data interpreter.
...
Preferably you can use Unitys Start method to initialize the MobileWebControl but you can also call the MobileWebControl instance at any other point of time which fits for your applications needs.
...
Secondly create a GameObject on your entry scene and add YourMobileWebControlInitializer as well as the Dispatcher script from the MobileWebControl/Network folder to it.
...
You can use your Initializer in all scenes, since it is designed as (Unity) singleton the initialization will be executed only once. Moreover the MobileWebController is aware of its internal state and aborts further calls to Initialize if setup was already executed previously.
...
From the base index.html file (located in MobileWebControl/WebResources) or from [demo project]()s index.html you will need to add the referenced scripts your HTML file.
Specifically you will need the webrtc.js and mobile-web-control.js files from the WebResources folder.
...
mobile-web-control.js initializeConnection function requires an object to be passed to it which handles the messages from the webrtc library.
...
If you are willing to use your own Webserver and/or WebRTC implementation there is no need to inherit from MobileWebControlInitializer. You can call the static MobileWebController instance and it's Initialize method from anywhere else in your application and pass in your custom implementations.

To use a custom servers you need to:

- implement the IWebServer interface
- implement the IWebRTC interface
- Pass instances together with your data interpreter to MobileWebController
...
At the bottom you can find more information on the MobileWebControl class, which can also be used with your custom server implementations.
...
Internally a thread for seving the requests is fired up and the class internally determines the IP address which can be accessed via MobileWebController.instance.webServerAddress.

### WebRTCServer

Based on radiomans [WebRTC.net]() implementation this class handles the negotiation for the webrtc connection between the Unity application and a web browser.
...
Received messages are passed from the WebRTC callbacks, each WebRTC session (for a distinct client) runs on a seperate thread as well, are passed via the static MobileWebController instance the this plugin.
...
When the Unity application is shut down, the MobileWebController requests a CleanUp (e.g. closing all threads) on the registered server instances and rejects further retrieved messages.
